        ...
        for graphpath in tqdm(glob.glob(f'{QUERY_CORPUS}*.dot')):
            ...
            for ngram in query_ngrams[:100]:
                ...
                if match:
                    match_counter += 1
                    if top_k == 1:
                        GPT_feed = f"This is the set of operators in my scientific workflow: {'->'.join(incomplete_ngram)}. " \
                                       f"These are the sets of operators from {k} similar workflows: {['->'.join(x) for x in knn_paths]}. " \
                                       f"what is the next operator in my workflow according to the set of operators in similar workflows? just say one word."
                    else:
                        GPT_feed = f"This is the set of operators in my scientific workflow: {'->'.join(incomplete_ngram)}. " \
                                   f"These are the sets of operators from {k} similar workflows: {['->'.join(x) for x in knn_paths]}. " \
                                   f"what is the {top_k} possible next operators in my workflow according to the set of operators in similar workflows? just say {top_k} word."
                    llm_prediction = ollama.recommend(GPT_feed)

                    if missing_element in llm_prediction:
